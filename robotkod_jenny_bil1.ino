...
void onConnectionEstablished()
{
  client.subscribe("filip.cederblad@abbindustrigymnasium.se/send", [] (const String &payload)
  {
    ...
    if(payload=="starting car"){
        ...
        client.publish("filip.cederblad@abbindustrigymnasium.se/send", "Jenny connected");
      }
       ...
       client.publish("filip.cederblad@abbindustrigymnasium.se/send","Jenny"+sendbvelocity);
    }
    else {
        String sendvelocity;
        sendvelocity = (String)Velocity;
        client.publish("filip.cederblad@abbindustrigymnasium.se/bil1",sendvelocity);
      }
